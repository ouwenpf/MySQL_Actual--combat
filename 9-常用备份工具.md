...
./mydumper  -S /data/mysql/mysql3306/tmp/mysql3306.sock -x '^(?!(test|mysql|sys))'   -o /root/mydumper
./mydumper  -S /data/mysql/mysql3306/tmp/mysql3306.sock -x 'db1|db2'
...
```sh
# 全库备份
pg_dump -h172.16.0.16 -p47001  -U abc postgres> postgres.sql

# 只备份表结构
pg_dump -h172.16.0.16 -p47001  -U abc postgres -s> postgres.sql

# 只备份数据
pg_dump -h172.16.0.16 -p47001  -U abc postgres -a> postgres.sql
