...
正常授权只需要RELOAD,LOCK TABLES,SUPER,BACKUP_ADMIN(针对8.0)
...
8.0备份略有不同参数
备份：
	xtrabackup  --defaults-file=/data/mysql/mysql3306/my.cnf -S  /tmp/mysql3306.sock --target-dir=/data/backup  --backup
还原:
	xtrabackup   --target-dir=/data/backup  --prepare
	xtrabackup  --defaults-file=/data/mysql/mysql3306/my.cnf -S  /tmp/mysql3306.sock --target-dir=/data/backup  --copy-back|--move-back 
	删除ib_logfile*文件 才可以拷贝
如果此命令拷贝不了，直接cp /data/backup/2019-02-25_12-54-58 /data/mysql/mysql3306/data/
...
4.接受者设置捐赠者列表清单
mysql -uclone_user -h10.0.0.8.11 
