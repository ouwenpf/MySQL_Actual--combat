...
--rcr-shard-remap 9:18,8:17,7:16  shard导出对应关系
...
psql postgres://abc:abc@192.168.0.176:47001/dw01 -c "\copy  gxzc_zzzc_gdzc_java_en to    '/tmp/gxzc_zzzc_gdzc_java_en.csv' WITH delimiter ',' QUOTE '''' csv  NULL 'NULL' ;"

psql postgres://abc:abc@192.168.0.176:47001/test  -c "\copy  gxzc_zzzc_gdzc_java_en from '/tmp/gxzc_zzzc_gdzc_java_en.csv' WITH delimiter ',' QUOTE '''' csv   NULL 'NULL';"

如果默认不在public下面可以设置alter user user_name  set search_path to schema_name; 修改用户的默认schema
或者把命令写入文件
psql postgres://abc:abc@192.168.0.185:46998/postgres   -f export.sql
cat << EOF > export.sql
SET search_path TO spm;
\copy (SELECT * FROM supp_arrival_notice WHERE arrival_id = 1540) TO '/tmp/supp_arrival_notice.csv' WITH delimiter ',' QUOTE '''' csv NULL 'NULL';
EOF
...
encoding ：（默认为utf8）
NULL 'null_string':
导出的时候PostgreSQL 默认会将字段为NULL值视为空,,不是NULL也不是"",也不是''
如果指定如:NULL 'test'  导出csv格式中字段为NULL变成test
PostgreSQL 默认会将CSV文件中空字符串 ('') 视为 NULL,定义在文件中哪些值应该视为 NULL，而不是空字符串或其他特殊值。
QUOTE ：指定需要用什么符号来将指定列进行定界符(vlaue='vlaue')，指定一个数据值被引用时使用的引用字符。默认是双引号。 这必须是一个单一的单字节字符。只有使用 CSV格式时才允许这个选项。
