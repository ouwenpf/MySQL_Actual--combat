...
#!/bin/sh
#
base_dir="/application/percona-Server-5.6.40"
...
# 
/usr/local/mysql/scripts/mysql_install_db --basedir=/usr/local/mysql/ --datadir=/data/mysql/mysql$1/data/ --user=mysql
sleep 5
if [ $? -eq 0 ];then
...
# 
/usr/local/mysql/bin/mysqld_safe --defaults-file=/data/mysql/mysql$1/my.cnf & &> /dev/null
sleep 60

if /usr/local/mysql/bin/mysql -S /tmp/mysql$1.sock  -e 'select @@server_id;' &> /dev/null;then
...
./ps_tokudb_admin --enable  -S  /tmp/mysql$1.sock
if [ $? -eq 0 ];then
        netstat -lntup|grep $1
        mysql -S  /tmp/mysql$1.sock  -e 'show engines;'|grep 'TokuDB'
else
